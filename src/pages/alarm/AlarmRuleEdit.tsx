...
export default function AlertRuleForm({
  mode = "create",
  initialData,
  onSubmit = async () => {},          // ✅ 기본 no-op: prop 없어도 에러 없음
  onDelete,
  lockMetricInstanceOnEdit = true,     // ✅ 수정모드에서 주요 식별 필드 잠글지 여부(기본 잠금)
}: {
  mode?: Mode;
  initialData?: Partial<AlertRulePayload>;
  onSubmit?: (payload: AlertRulePayload) => void | Promise<void>; // ✅ optional
  ...
  const title = "알림 규칙 수정";
  const saveLabel ="규칙 수정" ;
    ...
    <div className="ar-root">
      <section className="vd-card3">
        ...
        <div className="ar-save">
          {mode === "edit" && onDelete && (
            <button className="al-btn al-btn--danger" onClick={onDelete} style={{ marginRight: 8 }}>
              삭제
            </button>
          )}
          <button className="al-btn" onClick={handleSave}>{saveLabel}</button>
        </div>
      </section>
    </div>
  );
